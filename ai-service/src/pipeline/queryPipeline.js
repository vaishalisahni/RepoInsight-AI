...
async function query({ faissIndexId, question, history, repoName }) {
  ...
  const ctx    = chunks.map(c =>
    `// ${c.filePath}${c.startLine ? ` (lines ${c.startLine}-${c.endLine})` : ''}\n${c.content}`
  ).join('\n\n---\n\n');

  const answer = await llm(
    `You are an expert code assistant for "${repoName}". Always cite file paths.`,
...
async function trace({ faissIndexId, entryPoint, functionName }) {
  ...
  const result = await llm(
    'Trace code execution. End with a Mermaid sequence diagram.',
...
async function impact({ faissIndexId, filePath }) {
  ...
  const result = await llm(
    'Senior architect performing change impact analysis.',
