...
async function query({ faissIndexId, question, history, repoName }) {
  ...
  const answer = await llm(
    `You are an expert code assistant for "${repoName}". Always cite file paths and line numbers when referencing code.`,
    PROMPTS.query(repoName, question, ctx),
    history
  );
...
async function explain({ filePath, selection, chunks, faissIndexId }) {
  ...
  const relCtx = rel.filter(c => c.filePath !== filePath).slice(0, 3)
    .map(c => `// ${c.filePath} [${c.language || ''}]\n${(c.content || '').slice(0, 300)}`).join('\n\n');

  const explanation = await llm(
    'You are a senior engineer explaining code clearly and concisely.',
    PROMPTS.explain(filePath, code.slice(0, 4000), relCtx)
  );

  return {
    explanation,
    filePath,
    relatedFiles: rel.filter(c => c.filePath !== filePath).map(c => c.filePath),
  };
}
...
async function impact({ faissIndexId, filePath }) {
  ...
  const result = await llm(
    ...
    PROMPTS.impact(filePath, ctx)
  );
  return { analysis: result, relatedFiles: chunks.map(c => c.filePath) };
}

async function generateSummary(ctx, techStack, languages) {
  const stackInfo = techStack?.frameworks?.length
    ? `Detected tech stack: ${techStack.frameworks.map(f => f.name).join(', ')}`
    : '';
  const langInfo = languages
    ? `Languages: ${Object.entries(languages).map(([l, n]) => `${l}(${n})`).join(', ')}`
    : '';

  return llm(
    'You are a senior technical writer creating concise developer summaries.',
    PROMPTS.summary(ctx.slice(0, 6000), stackInfo, langInfo)
  );
}
