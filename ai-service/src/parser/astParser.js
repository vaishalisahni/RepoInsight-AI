...
function parseFile(filePath) {
  ...
  if (source.length > 500000) {
    return {
      chunks: [{ content: source.slice(0, 3000), type: 'generic', name: null, startLine: 1, endLine: 50, imports: [], exports: [] }],
      imports: [], exports: []
    };
  }

  let tree;
  try { const parser = factory(); tree = parser.parse(source); }
  catch (e) {
    return {
      chunks: [{ content: source.slice(0, 3000), type: 'generic', name: null, startLine: 1, endLine: source.split('\n').length, imports: [], exports: [] }],
      imports: [], exports: []
    };
  }
  ...
  function visit(node) {
    switch (node.type) {
      case 'import_declaration': {
        ...
        imports.push(text);
        chunks.push({ content: text, type: 'import', name: null, startLine: node.startPosition.row + 1, endLine: node.endPosition.row + 1, imports: [text], exports: [] });
        return;
      }
      ...
      case 'method_definition': {
        ...
        if (content.length < 10000)
          chunks.push({ content, type: 'function', name, startLine: node.startPosition.row + 1, endLine: node.endPosition.row + 1, imports: [], exports: [] });
        return;
      }
      case 'arrow_function': {
      ...
      case 'class_declaration': {
        ...
        const name     = nameNode ? source.slice(nameNode.startIndex, nameNode.endIndex) : null;
        const content  = source.slice(node.startIndex, node.endIndex);
        chunks.push({ content: content.slice(0, 8000), type: 'class', name, startLine: node.startPosition.row + 1, endLine: node.endPosition.row + 1, imports: [], exports: [] });
        return;
      }
  ...
  if (chunks.length === 0) {
    chunks.push({ content: source.slice(0, 3000), type: 'generic', name: null, startLine: 1, endLine: source.split('\n').length, imports: [], exports: [] });
  }
