...
/**
 * POST /api/ingest
 * Body (JSON): { url: "https://github.com/owner/repo", branch: "main" }
 *   OR
 * Body (multipart): file (ZIP), name (string)
 */
router.post('/', upload.single('file'), async (req, res, next) => {
  try {
    ...
    if (url) {
      ...
      return res.status(400).json({ error: 'Provide either a GitHub URL or a ZIP file.' });
    }
    ...
    // Non-blocking: call AI service to index
...
router.get('/', async (req, res, next) => {
  try {
    const repos = await Repo.find({}).select('name status totalFiles totalChunks createdAt url');
    res.json(repos);
  } catch (err) {
    next(err);
  }
