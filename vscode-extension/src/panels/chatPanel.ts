...
export class ChatPanel {
  ...
  private _activeRepoId: string | undefined;
  ...
  async traceFlow(entryPoint: string, functionName?: string) {
  ...
  // ── API calls ───────────────────────────────────────────────────────────
  private async _runQuery(question: string) {
  ...
  private async _runExplain(filePath: string, selection?: any) {
    if (!this._activeRepoId) {
      this._post({ type: 'error', message: 'Select a repository first.' });
      return;
    }
  ...
  private async _runImpact(filePath: string) {
    if (!this._activeRepoId) {
      this._post({ type: 'error', message: 'Select a repository first.' });
      return;
    }
  ...
  private async _runTrace(entryPoint: string, functionName?: string) {
    if (!this._activeRepoId) {
      this._post({ type: 'error', message: 'Select a repository first.' });
      return;
    }
  ...
  private async _sendInitialState() {
    ...
    if (isLoggedIn) await this._sendRepos();
  }

  private async _sendRepos() {
    try {
  ...
  private _openFileInEditor(filePath: string, line?: number) {
    const folders = vscode.workspace.workspaceFolders;
