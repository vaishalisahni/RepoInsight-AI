...
import {
  Brain, MessageSquare, GitBranch, Zap, Home, Plus, Code2,
...
export default function Sidebar() {
  const { repos, activeRepoId, activeTab, sidebarOpen,
          setRepos, setActiveRepo, setActiveTab, setGraphData, toggleSidebar } = useAppStore();
  ...
  return (
    <aside className={`relative flex flex-col shrink-0 transition-all duration-300 ${sidebarOpen ? 'w-56' : 'w-14'}`}
           style={{ background: 'rgba(13,11,30,0.95)', borderRight: '1px solid rgba(124,127,245,0.1)', backdropFilter: 'blur(20px)' }}>
      ...
      >
        {sidebarOpen ? <ChevronLeft className="w-3 h-3 text-ink-400" /> : <ChevronRight className="w-3 h-3 text-ink-400" />}
      </button>

      {/* Logo */}
      <div className={`px-3 py-4 flex items-center gap-2.5 border-b overflow-hidden`}
           style={{ borderColor: 'rgba(124,127,245,0.1)' }}>
        <div className="w-8 h-8 rounded-xl shrink-0 bg-gradient-to-br from-ink-500 to-cyan-500 flex items-center justify-center shadow-[0_0_16px_rgba(92,91,232,0.4)]">
      ...
      {/* Dashboard tabs (only when a repo is active) */}
