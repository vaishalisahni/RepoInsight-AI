...
import {
  ...
  ChevronRight, Sparkles, Search, ArrowRight, X
} from 'lucide-react';
...
// ── FileDetailPanel ─────────────────────────────────────────────────────────
function FileDetailPanel({ filePath, repoId, onAskInChat, onRunImpact, onClose }) {
              ...
              {parts.map((part, i) => (
                <span key={i} className="flex items-center gap-0.5">
                  <span
                    ...
                    style={{ color: i === parts.length - 1 ? 'var(--text-primary)' : 'var(--text-faint)', fontWeight: i === parts.length - 1 ? 600 : 400 }}
                  >
                    {part}
                  </span>
                  {i < parts.length - 1 && <ChevronRight className="w-2.5 h-2.5 shrink-0" style={{ color: 'var(--text-faint)' }} />}
                </span>
              ))}
            </div>
            <span
              className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full"
              style={{ background: `${dotColor}15`, border: `1px solid ${dotColor}30`, color: dotColor }}
            >
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4" style={{ minHeight: 0 }}>
        ...
        {explanation && (
          <div className="card-glass rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3 pb-2" style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>AI Explanation</p>
            </div>
        ...
        <div className="card-glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Quick Questions</p>
          </div>
          <div className="space-y-2">
            {quickQuestions.map((q, i) => (
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.border = '1px solid var(--accent-border)'; e.currentTarget.style.background = 'var(--bg-200)'; }}
                onMouseLeave={e => { e.currentTarget.style.border = '1px solid var(--border)'; e.currentTarget.style.background = 'var(--bg-100)'; }}
              >
                <span className="text-[12px]" style={{ color: 'var(--text-secondary)' }}>{q}</span>
                <ArrowRight className="w-3.5 h-3.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: 'var(--accent)' }} />
              </button>
            ))}
          </div>
        </div>
...
function ExplorerEmptyState({ activeRepo }) {
  return (
    <div className="flex flex-col items-center justify-center text-center px-8 h-full">
      ...
      <p className="text-[14px] font-semibold mb-1" style={{ fontFamily: "'Space Grotesk', sans-serif", color: 'var(--text-primary)' }}>
        File Explorer
      </p>
      <p className="text-[12px] max-w-xs leading-relaxed mb-6" style={{ color: 'var(--text-muted)' }}>
        Browse the file tree in the sidebar. Click any file to explain it, ask questions, or run impact analysis.
      </p>
      {activeRepo && (
      ...
      {activeRepo?.keyFiles?.length > 0 && (
        <div className="w-full max-w-sm">
          <p className="text-[10px] uppercase tracking-wider font-semibold mb-3" style={{ color: 'var(--text-faint)' }}>
            Key Files — click in sidebar to explore
          </p>
          <div className="space-y-1.5">
            {activeRepo.keyFiles.slice(0, 6).map((f, i) => {
              ...
              return (
                <div key={i} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl"
                  style={{ background: 'var(--bg-100)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex h-full">
        ...
        <div className="flex-1 flex items-center justify-center text-center px-4" style={{ paddingBottom: '60px' }}>
          <div>
            ...
            <p className="text-[12px] mt-1" style={{ color: 'var(--text-faint)' }}>Go home and index a repository first.</p>
          </div>
        </div>
    ...
    <div className="flex overflow-hidden" style={{ height: '100%' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {activeRepo?.techStack?.frameworks?.length > 0 && (
          ...
          >
            <span className="text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap" style={{ color: 'var(--text-faint)' }}>Stack:</span>
            <div className="flex items-center gap-2 flex-nowrap">
              {activeRepo.techStack.frameworks.map(fw => <TechStackBadge key={fw.name} framework={fw} />)}
            </div>
            {activeRepo.techStack.primaryLanguage && (
              <span className="ml-auto text-[10px] font-mono px-2 py-0.5 rounded-md whitespace-nowrap shrink-0"
        ...
        <div
          ...
          style={{ paddingBottom: activeRepoId ? 'calc(var(--mobile-nav-height, 0px) + env(safe-area-inset-bottom))' : 0 }}
        >
          {/* ── CHAT TAB ── */}
          {activeTab === 'chat' && (
            <div className="flex h-full overflow-hidden">
              ...
              >
                <div className="px-4 py-3 shrink-0 flex items-center gap-2" style={{ borderBottom: '1px solid var(--border)' }}>
                  <span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--text-faint)' }}>Repository Overview</span>
                </div>
                <div className="flex-1 overflow-y-auto min-h-0"><RepoSummary /></div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'graph' && (
            <div className="h-full p-2 md:p-4">
              {!graphData ? (
                ...
                <DependencyGraph graphData={graphData} onNodeClick={id => console.log('Node clicked:', id)} />
              )}
            </div>
          )}
      ...
      <div className="mobile-bottom-nav md:hidden">
        {MOBILE_TABS.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => handleMobileTab(tab.id)} style={{ color: isActive ? '#60a5fa' : 'var(--text-muted)' }}>
              <tab.icon style={{ width: 20, height: 20 }} />
              <span>{tab.label}</span>
            </button>
          );
